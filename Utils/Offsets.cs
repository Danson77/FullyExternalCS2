...
public abstract class Offsets
{
    ...
    public static int m_flCurrentTime;
    ...
    public static async Task UpdateOffsets()
    {
        try
        {
            string? sourceDataDw = null;
            string? sourceDataClient = null;
            ...
            catch (Exception ex)
            {
                ...
                try
                {
                    ...
                    sourceDataDw = await File.ReadAllTextAsync(_localOffsetsPath);
                    sourceDataClient = await File.ReadAllTextAsync(_localClientPath);
            ...
            if (fromRemote)
            {
                ...
                await File.WriteAllTextAsync(_localOffsetsPath, sourceDataDw);
                await File.WriteAllTextAsync(_localClientPath, sourceDataClient);
                Console.WriteLine("[Offsets] Saved local cache.");
            }

            using var offsetsDoc = JsonDocument.Parse(sourceDataDw);
            using var clientDoc = JsonDocument.Parse(sourceDataClient);
